...
func TestPaillier(t *testing.T) {
	max, min := int(99999999999999999), int(-99999999999999999)
	// 密钥生成
	DefaultPrimeLength = 1024
	...
	for i := 0; i < 999999999999999999; i++ {
		...
		if randX < 0 {
			negativeRandX := new(big.Int).Mod(inputx, paillierPublicKey.N)
			x, err = paillierPrivateKey.Encrypt(negativeRandX)
		} else {
			x, err = paillierPrivateKey.Encrypt(inputx)
		}
		if err != nil {
		...
		minTN := new(big.Int).Mul(tN, big.NewInt(-1))
		res := new(big.Int).Add(tM, minTN)
		if inputx != res {
			t.Logf("inputx is : %d, res is : %e", inputx, res)
			break
		}
